      ...
      <ResponsiveContainer width="100%" height={height}>
        ...
        >
          ...
          <YAxis
            ...
            minTickGap={24}
            domain={
              yTicks ? [Math.min(...yTicks), Math.max(...yTicks)] : undefined
            }
          />
          <Tooltip
            cursor={{ fill: "rgba(201,240,62,0.06)" }}
            contentStyle={{
              background: V.cardAlt,
          ...
          <Bar dataKey="value" radius={[6, 6, 6, 6]} maxBarSize={22}>
            {data.map((d, i) => (
              <Cell
                key={i}
                fill={d.highlight ? V.accent : "rgba(201,240,62,0.35)"}
              />
            ))}
          </Bar>
        </BarChart>
  ...
  return (
    ...
    >
      <ResponsiveContainer width="100%" height={height}>
        <LineChart
          data={data}
          margin={{ top: 12, right: 28, bottom: 4, left: 4 }}
        >
          <CartesianGrid
            vertical={false}
            strokeDasharray="2 4"
