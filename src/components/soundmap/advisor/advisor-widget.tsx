...
export function AdvisorPanel({
  ...
  return (
    <motion.div
      ...
      className={
        embedded
          ? "advisor-embedded flex flex-col overflow-hidden v6-panel h-[min(760px,75dvh)] min-h-[420px]"
          : "fixed z-[70] flex flex-col overflow-hidden bg-background text-foreground shadow-xl inset-0 sm:inset-auto sm:bottom-6 sm:right-6 sm:h-[640px] sm:max-h-[85vh] sm:w-[420px] sm:rounded-lg sm:border sm:border-border"
      }
    >
      {/* Header */}
      <div className="relative flex items-center justify-between border-b border-[var(--border)] px-5 py-4">
        <div
          ...
          style={{
            background:
              "radial-gradient(120% 100% at 0% 0%, var(--accent-dim) 0%, transparent 55%)",
          }}
        />
        <div className="relative flex items-center gap-3">
          <div
            className="flex h-9 w-9 items-center justify-center r-control"
            style={{ background: "var(--accent-dim)", color: "var(--accent)" }}
          >
            <AudioWaveform size={17} strokeWidth={2} />
          </div>
          <div>
            <p className="text-[14px] font-semibold leading-none text-white tracking-[-0.01em]">
        ...
        {!hasMessages && (
          <div className="flex flex-col items-center text-center px-4 pt-6">
            ...
            <p className="text-base font-bold text-white">
              ¿En qué te ayudo con tu sistema?
            </p>
            <p className="mt-1.5 text-[12px] leading-relaxed text-zinc-400 max-w-[280px]">
              Conozco tu recinto, su acústica y tu equipo. Preguntame lo que
              quieras sobre tu sistema de audio.
            </p>
        ...
        <form onSubmit={handleSubmit} className="flex items-end gap-2">
          ...
          {isStreaming ? (
            ...
            <button
              type="submit"
              disabled={!input.trim()}
              className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-accent text-accent-foreground transition-all hover:bg-accent/90 active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
              aria-label="Enviar"
            >
              <Send size={16} />
            </button>
