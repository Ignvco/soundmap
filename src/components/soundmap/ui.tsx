...
const CARD_TONES: Record<CardTone, string> = {
  accent:
    "bg-[var(--surface-2)] text-[var(--accent)] border border-[var(--accent-ring)]",
  ...
  warning:
    "bg-[var(--surface-2)] text-[var(--warning)] border border-[var(--border)]",
  white:
  ...
  surface:
    "bg-[var(--surface-1)] text-foreground border border-[var(--border)]",
};
        ...
        <div className="text-right">
          ...
          <p
            className={cn(
              ...
              dark ? "text-muted-foreground" : "opacity-70",
            )}
          >
            {label}
          </p>
...
const STATUS_CONFIG: Record<
  StatusType,
  { color: string; dot: string; text: string }
> = {
  ready: {
    color: "bg-accent/12 border-accent/30",
    dot: "bg-accent",
    text: "text-accent",
  },
  standby: {
    color: "bg-warning/12 border-warning/30",
    dot: "bg-warning",
    text: "text-warning",
  },
  live: {
    color: "bg-live/12 border-live/30",
    dot: "bg-live live-pulse",
    text: "text-live",
  },
  warning: {
    color: "bg-warning/12 border-warning/30",
  ...
  error: {
    color: "bg-destructive/12 border-destructive/30",
    dot: "bg-destructive",
  ...
  info: {
    color: "bg-info/12 border-info/30",
    dot: "bg-info",
    text: "text-info",
  },
...
export function StatusPill({ status, label, className }: StatusPillProps) {
  ...
  return (
    <div
      className={cn(
        "inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[10px] font-medium uppercase tracking-[0.16em]",
        cfg.color,
        className,
      )}
    >
...
export function ProButton({
  ...
  const base =
    "inline-flex items-center justify-center gap-2 rounded-md font-medium cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed";
  const variants = {
  ...
  return (
    <div className="relative inline-flex items-center justify-center">
      ...
      <div className="absolute flex flex-col items-center">
        ...
        {sublabel && (
          <span className="text-[9px] text-muted-foreground uppercase tracking-[0.16em] mt-0.5">
            {sublabel}
          </span>
        )}
      </div>
...
export function ScreenShell({
  ...
  return (
    <div
      className={cn(
        compact
          ? "feature-screen wizard-screen"
          : "v6-workspace feature-screen",
        className,
      )}
    >
      <div className="screen-body max-w-[1400px] mx-auto">{children}</div>
    </div>
  );
}
...
interface BadgeProps {
  ...
  color?:
    ...
    | "purple"
    | "blue"
    | "red"; // legacy aliases (mapped below)
  className?: string;
  "data-testid"?: string;
}
    ...
    <div className={cn("relative overflow-hidden rounded-2xl", className)}>
      ...
      <div className="flex justify-between px-1 mt-1">
        {["20", "100", "1k", "10k", "20k"].map((f) => (
          <span key={f} className="text-[10px] font-mono text-muted-foreground">
            {f}
          </span>
        ))}
      </div>
