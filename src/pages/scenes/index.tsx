  ...
  return (
    ...
    >
      {/* Fila única. El rediseño pide biblioteca profesional, no grilla de
          ...
          pasar el mouse. */}
      <div className="scene-row flex items-center gap-3 py-3.5 border-b border-border">
        <button
          className="v6-button"
          onClick={onPreview}
          aria-label={`Ver ${scene.name} en 3D`}
        >
          <Box size={15} />
        </button>
        <button
        ...
        >
          <span
            className="h-9 w-9 hidden sm:flex items-center justify-center shrink-0"
            style={{
          ...
          <span className="min-w-0 flex-1">
            ...
            >
              {scene.room.name} ·{" "}
              {format(new Date(scene.createdAt), "d MMM yyyy")}
              <span className="hidden sm:inline">
                {" "}
                ·{" "}
                {formatDistanceToNow(new Date(scene.createdAt), {
                  addSuffix: true,
                })}
              </span>
            </span>
        ...
        <div
          className="flex items-center gap-1 shrink-0 md:opacity-0 md:group-hover:opacity-100"
        ...
        >
          ...
          <button
            ...
            onMouseEnter={(e) => {
              e.currentTarget.style.color = HUE_DANGER;
            }}
            onMouseLeave={(e) => {
      ...
      <AnimatePresence>
        {confirmDelete && (
          <motion.div
            ...
            style={{
              background: `${HUE_DANGER}08`,
              borderTop: `1px solid ${HUE_DANGER}22`,
            }}
          >
...
export default function Scenes() {
  ...
  const preview = sorted.find((s) => s.id === previewId);
  const visible = sorted.filter((s) =>
    `${s.name} ${s.room.name}`.toLowerCase().includes(query.toLowerCase()),
  );
  ...
  return (
    <PremiumShell
      ...
      right={
        <div className="flex items-center gap-3">
          ...
          <button
            className="v6-button"
            onClick={() => navigate("/design?step=save")}
            aria-label="Guardar una nueva escena"
          >
            <Plus size={16} />
          </button>
        </div>
    ...
    >
      ...
      >
        <SheetContent side="bottom" className="scene-preview-sheet">
          <SheetTitle>{preview?.name}</SheetTitle>
          <SheetDescription>
            Vista previa de la escena guardada.
          </SheetDescription>
          {preview && (
            <>
              <VenuePreview
                layout={preview.stageLayout}
                room={preview.room}
                tops={preview.tops}
                subs={preview.subs}
              ...
              <button
                ...
                onClick={() =>
                  handleLoad(preview.clientId ?? preview.id, preview.name)
                }
              >
                Cargar escena <ChevronRight size={15} />
              </button>
            </>
          )}
        </SheetContent>
      ...
      {scenes.length > 0 && (
        <input
          className="v6-panel w-full px-3 py-2 text-sm mb-4"
          aria-label="Buscar escenas"
      ...
      {sorted.length === 0 && (
        ...
        >
          <div
            className="inline-flex h-14 w-14 rounded-full items-center justify-center mb-5"
            style={{
              background: "rgba(255,255,255,0.03)",
          ...
          <p className="text-[13px] text-muted-foreground max-w-sm mx-auto leading-relaxed mb-6">
            Completá un escaneo de sala, configurá el gear y guardá una
            instantánea desde Exportar.
          </p>
          <button
            onClick={() => {
              feedback("select");
          ...
          <AnimatePresence>
            {visible.map((scene) => (
              <SceneCard
                ...
                onLoad={() =>
                  handleLoad(scene.clientId ?? scene.id, scene.name)
                }
