...
export default function ExportPage() {
  const { room, acoustics, tops, subs, monitors, dspUnits, amps, saveScene } =
    useAppStore();
...
`
  ...
  const handleShare = async () => {
    if (!room || !acoustics) {
      toast.error("Sin datos para compartir");
      return;
    }
    if (navigator.share) {
      try {
      ...
      await navigator.clipboard.writeText(exportContent);
      toast.success(
        "Copiado al portapapeles (compartir no disponible en este dispositivo)",
      );
    }
  };
  ...
  const handleSaveScene = () => {
    if (!sceneName.trim()) {
      toast.error("Ingresá un nombre de escena");
      return;
    }
    if (!room) {
      toast.error("Completá un Escaneo de Sala primero");
  ...
  return (
    <ScreenShell compact={inWizard} className="export-page-workspace">
      {inWizard && (
        <div className="step-intro">
          <h2>Guardá tu escena</h2>
          <p>Revisá el sistema y exportá la configuración.</p>
        </div>
      )}
      {!inWizard && (
        <PageHeader
          ...
          right={
            <StatusPill
              status={hasData ? "ready" : "offline"}
              label={hasData ? "Listo" : "Sin Datos"}
            />
          }
        />
      )}
      ...
      <div className={inWizard ? "space-y-3 pb-4" : "px-4 space-y-3 pb-4"}>
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground">
                Guardar toda esta configuración
              </p>
            </div>
          </div>
          <div className="save-scene-form flex gap-2">
            <input
              type="text"
              value={sceneName}
            ...
            <ProButton
              ...
              disabled={!hasData || !sceneName.trim()}
            >
              Guardar
            </ProButton>
          </div>
        ...
        {hasData && (
          ...
          >
            ...
            <MetricCard
              ...
              value={pa?.splTarget ?? "—"}
              unit="dB"
              sub="Techo del sistema"
            />
            <MetricCard
              label="Cobertura"
              value={`${stage?.coveragePercent ?? "—"}`}
              unit="%"
              sub="Área del escenario"
            />
          </motion.div>
        )}
        ...
        {hasData && (
          ...
          >
            <div className="mt-2 space-y-0">
              ...
              <DataRow
                label="Dimensiones"
                value={`${room!.length}m × ${room!.width}m × ${room!.height}m`}
              />
              ...
              <DataRow
                label="RT60 + Público"
                value={`${acoustics!.rt60Audience}s`}
              />
              <DataRow
                label="Frecuencia de Schroeder"
                value={`${acoustics!.schroederFreq} Hz`}
              />
              <DataRow
                label="Distancia Crítica"
                value={`${acoustics!.criticalDistance}m`}
              />
              <DataRow
                label="Riesgo de Eco"
                value={acoustics!.echoRisk.toUpperCase()}
                accent={acoustics!.echoRisk === "high"}
              />
              <DataRow
                label="Puntaje Musical"
                value={`${acoustics!.musicScore}/100`}
              />
            </div>
          </CollapseSection>
        )}
        ...
        {/* ── PA System ── */}
        {hasData && pa && (
          <CollapseSection title="Sistema PA" icon={<Speaker size={16} />}>
            <div className="mt-2 space-y-0">
          ...
          <CollapseSection title="Salidas DSP" icon={<Cpu size={16} />}>
            <div className="mt-2 space-y-2">
              {dsp.outputs.map((o, i) => (
                <div
                  key={i}
                  className="rounded-xl bg-secondary border border-border p-3"
                >
                  ...
                  <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-[11px]">
                    ...
                    <span className="text-muted-foreground">
                      LPF:{" "}
                      <span className="text-foreground font-medium">
                        {o.lpfHz >= 20000 ? "Completo" : o.lpfHz + " Hz"}
                      </span>
                    </span>
                    <span className="text-muted-foreground">
                      Limitador:{" "}
                      <span className="text-foreground font-medium">
                        {o.limiterDb.toFixed(1)} dB
                      </span>
                    </span>
                    <span className="text-muted-foreground">
                      Delay:{" "}
        ...
        {hasData && stage && (
          <CollapseSection
            title="Configuración de Escenario"
            icon={<Layers size={16} />}
          >
            <div className="mt-2 space-y-0">
              <DataRow
                label="Modo de Despliegue"
                value={stage.deploymentMode}
                accent
              />
              ...
              <DataRow
                ...
                value={
                  stage.needsDelayTowers
                    ? `Sí — ${stage.delayTowerDistance}m`
                    : "No requerido"
                }
              />
            </div>
        ...
        <GlassCard className="p-4" glow>
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground">
                Descargá el PDF completo o copiá en markdown
              </p>
            </div>
          </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <GlassCard className="p-8 text-center">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                Completá un Escaneo de Sala y configurá tu equipo para generar
