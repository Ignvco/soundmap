...
function HPFBar({ hz }: { hz: number }) {
  const pct =
    ((Math.log10(hz) - Math.log10(20)) / (Math.log10(20000) - Math.log10(20))) *
    100;
  return (
      ...
      >
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              {ch.priority === "critical" && (
                <span className="shrink-0 text-[8px] font-bold uppercase tracking-[0.2em] text-accent bg-accent/12 px-1.5 py-0.5 rounded">
                  CRIT
                </span>
              )}
            </div>
          ...
          <div className="flex flex-col items-end gap-1 shrink-0 ml-1">
            {ch.phantom && (
              <span className="text-[8px] font-bold bg-chart-2/15 text-chart-2 border border-chart-2/25 px-1.5 py-0.5 rounded-md">
                48V
              </span>
            )}
            <div
              ...
              <div className="px-3 pb-3 pt-0 border-t border-border">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-2.5 pt-3 mb-3">
                  ...
                  <div>
                    ...
                    <p
                      className={`text-sm font-bold ${ch.phantom ? "text-chart-2" : "text-muted-foreground"}`}
                    >
                  ...
                  <div>
                    ...
                    >
                      {FEEDBACK_LABEL[ch.feedbackRisk]}
                    </p>
                  </div>
                </div>
                ...
                <div className="mb-3">
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <p className="text-[11px] font-bold text-chart-2">
                      {ch.hpfHz} Hz
                    </p>
                  </div>
                  <HPFBar hz={ch.hpfHz} />
                ...
                <div className="rounded-xl bg-secondary border border-border px-3 py-2.5 mb-2">
                  <p className="text-[9px] text-muted-foreground uppercase tracking-[0.28em] mb-1">
                    Sugerencia EQ
                  </p>
...
function ChannelStrip({
  ...
  return (
    <div
      ...
      style={{
        borderColor:
          ch.priority === "critical" ? `${priorityColor}40` : undefined,
      }}
    >
      {/* Feedback dot */}
      <div
        className="self-end h-1.5 w-1.5 rounded-full"
        style={{ background: feedbackColor }}
      />
        ...
        {[70, 55, 65, 50, 60].map((h, i) => {
          ...
          return (
            <motion.div
              ...
              style={{
                backgroundColor: isActive ? color : "rgba(255,255,255,0.10)",
              }}
      ...
      <div className="w-0.5 h-8 bg-secondary rounded-full relative mx-auto">
        <div
          ...
          style={{
            height: "65%",
            background: `linear-gradient(to top, ${priorityColor}, ${priorityColor}60)`,
          }}
        />
        <div
      ...
      <p className="text-[7px] text-muted-foreground text-center leading-none truncate w-full">
        {ch.name}
      </p>
      {ch.phantom && (
        <div className="text-[6px] font-bold text-chart-2 bg-chart-2/12 px-1 rounded">
          48V
        </div>
      )}
    </div>
...
export default function Channels() {
  ...
  const filtered = useMemo(() => {
    switch (filter) {
      case "critical":
        return allChannels.filter((c) => c.priority === "critical");
      case "phantom":
        return allChannels.filter((c) => c.phantom);
      case "feedback":
        return allChannels.filter((c) => c.feedbackRisk !== "low");
      default:
        return allChannels;
    }
  }, [allChannels, filter]);
  ...
  return (
    <ScreenShell compact={inWizard} className="channels-workspace">
      {inWizard && (
        <div className="step-intro">
          <h2>Patch de canales</h2>
          <p>Entradas, micrófonos y asignación de señales.</p>
        </div>
      )}
      {!inWizard && (
        <PageHeader
      ...
      <MetricRow className="patch-statistics mb-5">
        <Metric
          size="md"
          label="Críticos"
        ...
        <Metric
          size="md"
          label="48V Phantom"
          value={String(phantomCount)}
          testId="patch-stat-phantom"
        />
        <Metric
          size="md"
          label="Riesgo FB"
          value={String(fbCount)}
      ...
      <div className="mb-4 flex items-center gap-2">
        <div
          className="flex gap-1 flex-1 rounded-full p-1"
          style={{
            background: "rgba(255,255,255,0.03)",
            boxShadow: `0 0 0 1px ${V.hairline}`,
          }}
        >
          {(["all", "critical", "phantom", "feedback"] as ChannelFilter[]).map(
            (f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
