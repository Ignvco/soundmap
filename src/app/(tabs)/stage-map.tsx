...
import type { AmpConfig, GearSelection } from '@/store/app';
...
function MapSVG({
  ...
  return (
    <Svg width={W} height={H}>
      ...
      <Polygon
        points={`${tIzqX},${topY + 10} ${sx + 12},${sy + sh - 8} ${tipX - 4},${sy + sh - 8}`}
        fill="rgba(26,255,110,0.05)"
      />
      <Polygon
        points={`${tDerX},${topY + 10} ${sx + sw - 12},${sy + sh - 8} ${tipX + 4},${sy + sh - 8}`}
        fill="rgba(26,255,110,0.05)"
      />
      <Rect
        x={tIzqX - 11}
...
export default function StageMapScreen() {
  ...
  const isSelected = (item: GearItem) => {
    switch (item.cat) {
      case 'tops':
        return (gear.tops ?? []).includes(item.id)
      case 'subs':
        return (gear.subs ?? []).includes(item.id)
      case 'amps':
        return gear.amps.some((a) => a.ampId === item.id)
      case 'dsp':
        return gear.dsps.some((d) => d.dspId === item.id)
      case 'mixer':
        return gear.mixer === item.id
      case 'mics':
        return (gear.mics ?? []).includes(item.id)
      case 'monitores':
        return (gear.monitores ?? []).includes(item.id)
      default:
        return false
    }
  }
      ...
      <View style={{ paddingHorizontal: 16, paddingTop: 60, paddingBottom: 14 }}>
        ...
        <View
          style={{
            ...
            alignItems: 'center',
            gap: 8,
            backgroundColor: `${sysColor}10`,
            borderRadius: 10,
            borderWidth: 0.5,
            borderColor: `${sysColor}30`,
            padding: 10,
            marginBottom: 14,
          }}
        >
          <View
            style={{
              ...
              shadowColor: sysColor,
              shadowRadius: 4,
              shadowOpacity: 0.8,
            }}
          />
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 12, fontWeight: '700', color: sysColor }}>
              {SYS_LABEL[gear.systemType]}
            </Text>
            <Text style={{ fontSize: 10, color: T2, marginTop: 1 }}>
              {SYS_DESC[gear.systemType]}
            </Text>
          </View>
          {/* Override manual */}
              ...
              >
                <Text
                  style={{
                    fontSize: 8,
                    fontWeight: '700',
                    color: gear.systemType === t ? SYS_COLOR[t] : T3,
                  }}
                >
        ...
        <View style={{ flexDirection: 'row', gap: 6 }}>
          {[
            ...
            >
              <Text style={{ fontSize: 11, fontWeight: '700', color: tab === k ? AC : T2 }}>
                {l}
              </Text>
            </TouchableOpacity>
          ))}
              ...
              return (
                <View
                  ...
                  style={{
                    ...
                    paddingVertical: 2,
                    borderRadius: 20,
                    backgroundColor: item.active ? C.ac2 : C.bl2,
                    borderWidth: 0.5,
                    borderColor: item.active ? 'rgba(26,255,110,0.2)' : 'rgba(96,165,250,0.2)',
                  }}
                >
                  <Text style={{ fontSize: 8, fontWeight: '700', color: item.active ? AC : BL }}>
            ...
            >
              ...
              <View style={{ flexDirection: 'row', gap: 5 }}>
                {(['frontal', 'planta'] as MapV[]).map((v) => (
                  ...
                  >
                    <Text
                      style={{ fontSize: 10, fontWeight: '600', color: mapView === v ? AC : T2 }}
                    >
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
            {[
              ...
              >
                ...
                <Text style={{ fontSize: 11, fontWeight: '700', color: c }} numberOfLines={1}>
                  {v}
                </Text>
              </View>
            ))}
      ...
      {tab === 'gear' && (
        <View>
          ...
          {gear.systemType !== 'active' && gear.amps.length === 0 && (
            ...
            >
              <Text style={{ fontSize: 11, fontWeight: '700', color: RD }}>
                ⚠ Sistema pasivo — seleccioná al menos un amplificador en la tab Amps
              </Text>
            </View>
          )}
          ...
          >
            ...
            {[...gear.tops, ...gear.subs, gear.dsp, gear.mixer].filter(Boolean).map((id) => {
              ...
              return (
                ...
                >
                  <Text style={{ fontSize: 8, fontWeight: '700', color: AC }}>
                    {item.name.split(' ').slice(0, 3).join(' ')}
                  </Text>
                </View>
              )
          ...
          >
            {GEAR_CATS.map(({ key, label }) => (
              ...
              >
                <Text style={{ fontSize: 11, fontWeight: '600', color: gearCat === key ? AC : T2 }}>
                  {label}
                </Text>
              </TouchableOpacity>
            ))}
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    >
                      ...
                      >
                        <Text
                          style={{ fontSize: 7, fontWeight: '700', color: item.active ? AC : BL }}
                        >
                          {item.active ? 'ACTIVO' : 'PASIVO'}
                        </Text>
                      </View>
                    </View>
                    <Text
        ...
        <View style={{ paddingHorizontal: 14 }}>
          {gear.systemType === 'active' && (
            ...
            >
              <Text style={{ fontSize: 11, color: AC, fontWeight: '600' }}>
                Sistema activo — los amplificadores son internos en cada parlante. Los amps externos
                son opcionales (monitor pasivo, etc.).
              </Text>
            </View>
          )}
          ...
          {gear.amps.length === 0 ? (
            ...
            >
              ...
              <Text style={{ fontSize: 12, color: T2, textAlign: 'center' }}>
                No hay amplificadores configurados.
              </Text>
              <Text style={{ fontSize: 11, color: T3, textAlign: 'center', marginTop: 4 }}>
                Seleccioná un amp en la tab Gear → Amplif.
              </Text>
            </View>
          ) : (
              ...
              return (
                <View
                  ...
                  style={{
                    ...
                    borderRadius: 16,
                    borderWidth: 0.5,
                    borderColor: compat.ok ? BR : 'rgba(255,77,77,0.3)',
                    padding: 14,
                    marginBottom: 10,
                  }}
                >
                  ...
                  {compat.warning && (
                    <View
                      style={{
                        backgroundColor: compat.ok ? C.am2 : C.rd2,
                        borderRadius: 8,
                        padding: 9,
                        marginBottom: 10,
                        borderWidth: 0.5,
                        borderColor: compat.ok ? 'rgba(251,191,36,0.25)' : 'rgba(255,77,77,0.25)',
                      }}
                    >
                      <Text style={{ fontSize: 11, color: compat.ok ? AM : RD }}>
                        {compat.warning}
                      </Text>
                    </View>
                  )}
          ...
          {GEAR_DB.filter((g) => g.cat === 'amps').map((item) => (
            <TouchableOpacity
              ...
              onPress={() => toggleItem(item)}
              style={{
                backgroundColor: isSelected(item) ? C.ac2 : C.glass,
                borderRadius: 12,
                borderWidth: 0.5,
                borderColor: isSelected(item) ? 'rgba(26,255,110,0.35)' : BR,
                padding: 12,
                marginBottom: 7,
                flexDirection: 'row',
                alignItems: 'center',
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text
                  style={{ fontSize: 12, fontWeight: '600', color: isSelected(item) ? AC : TX }}
                >
                  {item.name}
                </Text>
                <Text style={{ fontSize: 10, color: T2 }}>{item.specs}</Text>
